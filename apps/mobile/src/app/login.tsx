...
export default function Login() {
  ...
  const submit=async()=>{ setError(''); setLoading(true); try { const r=await api.login(email.trim(),password); await signIn(r.accessToken,r.user); router.replace('/'); } catch(e){ setError(e instanceof Error?e.message:'Unable to sign in'); } finally { setLoading(false); }};
  return <SafeAreaView style={s.safe}><View style={s.card}><Text style={s.brand}>Memora</Text><Text style={s.title}>Welcome back</Text><Text style={s.sub}>Sign in to continue preserving what matters.</Text>
    ...
    <Text style={s.footer}>New to Memora? <Link href="/register" style={s.link}>Create account</Link></Text></View></SafeAreaView>;
}
